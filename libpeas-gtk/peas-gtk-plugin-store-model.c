...
peas_gtk_plugin_store_model_constructed (GObject *object)
{
  ...
  if (G_OBJECT_CLASS (peas_gtk_plugin_store_model_parent_class)->constructed != NULL)
...
peas_gtk_plugin_store_model_dispose (GObject *object)
{
  ...
  if (model->priv->engine != NULL)
    {
      g_object_unref (model->priv->engine);
      model->priv->engine = NULL;
    }
 ...
 * @plugins: A #GPtrArray of #PeasGtkInstallablePluginInfos.
 *
