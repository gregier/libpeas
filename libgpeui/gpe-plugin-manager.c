...
#define GPE_PLUGIN_MANAGER_GET_PRIVATE(object)(G_TYPE_INSTANCE_GET_PRIVATE ((object), GPE_TYPE_PLUGIN_MANAGER, GPEPluginManagerPrivate))

struct _GPEPluginManagerPrivate
{
	GtkWidget	*tree;
...
G_DEFINE_TYPE(GPEPluginManager, gpe_plugin_manager, GTK_TYPE_VBOX)

static GPEPluginInfo *plugin_manager_get_selected_plugin (GPEPluginManager *pm);
static void plugin_manager_toggle_active (GPEPluginManager *pm, GtkTreeIter *iter, GtkTreeModel *model);
static void gpe_plugin_manager_constructed (GObject *object);
static void gpe_plugin_manager_finalize (GObject *object);

static void
gpe_plugin_manager_set_property (GObject      *object,
				 guint         prop_id,
				 const GValue *value,
				 GParamSpec   *pspec)
{
	GPEPluginManager *pm = GPE_PLUGIN_MANAGER (object);
...
static void
gpe_plugin_manager_get_property (GObject    *object,
				 guint       prop_id,
				 GValue     *value,
				 GParamSpec *pspec)
{
...
static void
gpe_plugin_manager_class_init (GPEPluginManagerClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);

	object_class->set_property = gpe_plugin_manager_set_property;
	object_class->get_property = gpe_plugin_manager_get_property;
	object_class->constructed = gpe_plugin_manager_constructed;
	object_class->finalize = gpe_plugin_manager_finalize;

	g_object_class_install_property (object_class, PROP_ENGINE,
					 g_param_spec_object ("engine",
							      "engine",
							      "The GPEEngine this manager is attached to",
							      GPE_TYPE_ENGINE,
							      G_PARAM_READWRITE |
							      G_PARAM_CONSTRUCT_ONLY |
							      G_PARAM_STATIC_STRINGS));

	g_type_class_add_private (object_class, sizeof (GPEPluginManagerPrivate));
}
		 ...
		 GPEPluginManager *pm)
{
	...
	pm->priv->about = g_object_new (GTK_TYPE_ABOUT_DIALOG,
		"program-name", gpe_plugin_info_get_name (info),
		"copyright", gpe_plugin_info_get_copyright (info),
		"authors", gpe_plugin_info_get_authors (info),
		"comments", gpe_plugin_info_get_description (info),
		"website", gpe_plugin_info_get_website (info),
		"logo-icon-name", gpe_plugin_info_get_icon_name (info),
		"version", gpe_plugin_info_get_version (info),
		NULL);
		     ...
		     GPEPluginManager *pm)
{
	...
	gpe_engine_configure_plugin (pm->priv->engine, info, toplevel);
}
				  ...
				  gpointer           data)
{
	...
	text = g_markup_printf_escaped ("<b>%s</b>\n%s",
					...
					gpe_plugin_info_get_description (info));
	g_object_set (G_OBJECT (cell),
		      "markup", text,
		      "sensitive", gpe_plugin_info_is_available (info),
		      NULL);
				  ...
				  gpointer           data)
{
	...
	g_object_set (G_OBJECT (cell),
		      "icon-name", gpe_plugin_info_get_icon_name (info),
		      "sensitive", gpe_plugin_info_is_available (info),
		      NULL);
}
		   ...
		   gpointer     data)
{
	...
	gtk_widget_set_sensitive (GTK_WIDGET (pm->priv->configure_button),
				  (info != NULL) &&
				   gpe_plugin_info_is_configurable (info));
}
...
static void
plugin_manager_populate_lists (GPEPluginManager *pm)
{
	const GList *plugins;
	GtkListStore *model;
	GtkTreeIter iter;

	plugins = gpe_engine_get_plugin_list (pm->priv->engine);
	...
	{
		GPEPluginInfo *info;
		info = (GPEPluginInfo *)plugins->data;

		gtk_list_store_append (model, &iter);
		gtk_list_store_set (model, &iter,
				    ACTIVE_COLUMN, gpe_plugin_info_is_active (info),
				    AVAILABLE_COLUMN, gpe_plugin_info_is_available (info),
				    INFO_COLUMN, info,
				    -1);
...
static gboolean
plugin_manager_set_active (GPEPluginManager *pm,
			   GtkTreeIter      *iter,
			   GtkTreeModel     *model,
			   gboolean          active)
{
	...
	{
		/* activate the plugin */
		if (!gpe_engine_activate_plugin (pm->priv->engine, info))
			res = FALSE;
	}
	else
	{
		/* deactivate the plugin */
		if (!gpe_engine_deactivate_plugin (pm->priv->engine, info))
			res = FALSE;
	}
...
static void
plugin_manager_toggle_active (GPEPluginManager *pm,
			      GtkTreeIter      *iter,
			      GtkTreeModel     *model)
{
	gboolean active;
...
plugin_manager_get_selected_plugin (GPEPluginManager *pm)
{
	GPEPluginInfo *info = NULL;
	GtkTreeModel *model;
	GtkTreeIter iter;
	GtkTreeSelection *selection;
...
static void
plugin_manager_set_active_all (GPEPluginManager *pm,
			       gboolean          active)
{
	GtkTreeModel *model;
	GtkTreeIter iter;
		...
		gpointer      data)
{
	...
	normalized_string = g_utf8_normalize (gpe_plugin_info_get_name (info), -1, G_NORMALIZE_ALL);
...
static GtkWidget *
create_tree_popup_menu (GPEPluginManager *pm)
{
	GtkWidget *menu;
	GtkWidget *item;
	GtkWidget *image;
	...
	gtk_widget_set_sensitive (item, gpe_plugin_info_is_configurable (info));
	gtk_menu_shell_append (GTK_MENU_SHELL (menu), item);

	item = gtk_check_menu_item_new_with_mnemonic (_("A_ctivate"));
	gtk_widget_set_sensitive (item, gpe_plugin_info_is_available (info));
...
static void
tree_popup_menu_detach (GPEPluginManager *pm,
			GtkMenu            *menu)
{
...
model_name_sort_func (GtkTreeModel *model,
		      ...
		      gpointer      user_data)
{
	GPEPluginInfo *info1, *info2;

	gtk_tree_model_get (model, iter1, INFO_COLUMN, &info1, -1);
	gtk_tree_model_get (model, iter2, INFO_COLUMN, &info2, -1);

	return g_utf8_collate (gpe_plugin_info_get_name (info1),
			       gpe_plugin_info_get_name (info2));
}

static void
plugin_manager_construct_tree (GPEPluginManager *pm)
{
	GtkTreeViewColumn *column;
	GtkCellRenderer *cell;
	GtkListStore *model;
...
static void
plugin_toggled_cb (GPEEngine        *engine,
		   GPEPluginInfo    *info,
		   GPEPluginManager *pm)
{
	GtkTreeSelection *selection;
	...
	if (!info_found)
	{
		g_warning ("GPEPluginManager: plugin '%s' not found in the tree model",
			   gpe_plugin_info_get_name (info));
		return;
	}

	gtk_list_store_set (GTK_LIST_STORE (model), &iter, ACTIVE_COLUMN, gpe_plugin_info_is_active (info), -1);
}

static void
gpe_plugin_manager_init (GPEPluginManager *pm)
{
	GtkWidget *label;
	GtkWidget *viewport;
	GtkWidget *hbuttonbox;

	pm->priv = GPE_PLUGIN_MANAGER_GET_PRIVATE (pm);
...
static void
gpe_plugin_manager_constructed (GObject *object)
{
	GPEPluginManager *pm = GPE_PLUGIN_MANAGER (object);

	/* When we create the manager, we always rescan the plugins directory */
	gpe_engine_rescan_plugins (pm->priv->engine);
	...
	if (gpe_engine_get_plugin_list (pm->priv->engine) != NULL)
	{
...
static void
gpe_plugin_manager_finalize (GObject *object)
{
	GPEPluginManager *pm = GPE_PLUGIN_MANAGER (object);
...
{
	return GTK_WIDGET (g_object_new (GPE_TYPE_PLUGIN_MANAGER, "engine", engine, NULL));
}
