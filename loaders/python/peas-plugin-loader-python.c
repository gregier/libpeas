...
#define PEAS_PLUGIN_LOADER_PYTHON_GET_PRIVATE(object)(G_TYPE_INSTANCE_GET_PRIVATE((object), PEAS_TYPE_PLUGIN_LOADER_PYTHON, PeasPluginLoaderPythonPrivate))
...
PEAS_PLUGIN_LOADER_REGISTER_TYPE (PeasPluginLoaderPython, peas_plugin_loader_python, G_TYPE_OBJECT, peas_plugin_loader_iface_init);
					      ...
					      const gchar     *module_dir)
{
	...
	peas_plugin_loader_python_add_module_path (PEAS_PLUGIN_LOADER_PYTHON (loader),
						  module_dir);
}
			      ...
			      PeasPluginInfo   *info)
{
	...
	peas_plugin_loader_python_add_module_path (pyloader, peas_plugin_info_get_module_dir (info));
...
peas_init_libpeas (void)
{
	...
	{
		PyErr_SetString (PyExc_ImportError,
				 "could not import libpeas module");
		return;
	}
	...
	{
		PyErr_SetString (PyExc_ImportError,
				 "libpeas module version too old");
		Py_DECREF (required_version);
		return;
	}
...
peas_python_init (PeasPluginLoaderPython *loader)
{
	...
	if (PyErr_Occurred ())
	{
		...
		g_warning ("Error initializing Python interpreter: could not import libpeas module.");
...
{
	self->priv = PEAS_PLUGIN_LOADER_PYTHON_GET_PRIVATE (self);
