		  ...
		  const gchar *data_dir)
{
	...
	info = _peas_plugin_info_new (filename, engine->priv->app_name, module_dir, data_dir);
	...
	if (peas_engine_get_plugin_info (engine, peas_plugin_info_get_module_name (info)) != NULL)
		    ...
		    const gchar *loader_id)
{
	...
	module = peas_object_module_new (loader_basename, loader_dirname,
					"register_peas_plugin_loader", TRUE);
				 ...
				 PeasPluginInfo *info)
{
	...
	for (item = engine->priv->object_list; item != NULL; item = item->next)
		PEAS_ENGINE_GET_CLASS (engine)->activate_plugin_on_object (engine, info, G_OBJECT (item->data));
}
				   ...
				   PeasPluginInfo *info)
{
	...
	for (item = engine->priv->object_list; item != NULL; item = item->next)
		PEAS_ENGINE_GET_CLASS (engine)->deactivate_plugin_on_object (engine, info, G_OBJECT (item->data));
		       ...
		       GObject   *object)
{
	...
	for (pl = engine->priv->plugin_list; pl; pl = pl->next)
	{
		...
		PEAS_ENGINE_GET_CLASS (engine)->activate_plugin_on_object (engine, info, object);
	}
			  ...
			  GObject   *object)
{
	...
	for (pl = engine->priv->plugin_list; pl; pl = pl->next)
	{
		...
		PEAS_ENGINE_GET_CLASS (engine)->deactivate_plugin_on_object (engine, info, object);
	}
}
