...
#define PEAS_PLUGIN_MANAGER_GET_PRIVATE(object)(G_TYPE_INSTANCE_GET_PRIVATE ((object), PEAS_TYPE_PLUGIN_MANAGER, PeasPluginManagerPrivate))
...
static void plugin_manager_toggle_active (PeasPluginManager *pm, GtkTreeIter *iter, GtkTreeModel *model);
		   ...
		   PeasPluginManager *pm)
{
	...
	gtk_list_store_set (GTK_LIST_STORE (model), &iter, ACTIVE_COLUMN, peas_plugin_info_is_active (info), -1);
}
...
peas_plugin_manager_init (PeasPluginManager *pm)
{
	...
	pm->priv = PEAS_PLUGIN_MANAGER_GET_PRIVATE (pm);
...
{
	return GTK_WIDGET (g_object_new (PEAS_TYPE_PLUGIN_MANAGER, "engine", engine, NULL));
}
